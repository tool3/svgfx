...
import type { PstfxError } from '../src/core/errors.ts'
...
test('returns the document untouched when no effects are given', () => {
  assert.equal(pstfx(SOURCE, []), SOURCE)
})

test('adds the svg namespace when it is missing', () => {
  assert.match(pstfx('<svg viewBox="0 0 4 4"><rect/></svg>', []), /xmlns="http:\/\/www\.w3\.org\/2000\/svg"/)
})

test('is deterministic across identical calls', () => {
  const first = pstfx(SOURCE, [scanlines({ animate: true })])
  const second = pstfx(SOURCE, [scanlines({ animate: true })])
  assert.equal(first, second)
})

test('gives different documents different id namespaces', () => {
  const first = pstfx(SOURCE, [blur()])
  const second = pstfx(SOURCE.replace('100', '120'), [blur()])
  const idOf = (markup: string): string => /id="([^"]+)"/.exec(markup)?.[1] ?? ''
  assert.notEqual(idOf(first), idOf(second))
})

test('honours an explicit scope', () => {
  assert.match(pstfx(SOURCE, [blur()], { scope: 'hero' }), /id="pstfx-hero-filter-0"/)
})

test('honours a custom prefix', () => {
  assert.match(pstfx(SOURCE, [blur()], { prefix: 'acme' }), /id="acme-/)
})

test('merges consecutive filter effects into a single filter element', () => {
  const output = pstfx(SOURCE, [grayscale(), invert(), blur()])
...
test('splits filters that are separated by a layer effect', () => {
  const output = pstfx(SOURCE, [grayscale(), scanlines({ clip: 'viewport' }), blur()])
  assert.equal(count(output, '<filter '), 2)
})

test('chains filter primitives through the merged filter', () => {
  const output = pstfx(SOURCE, [grayscale(), invert()])
...
test('disables motion globally through settings', () => {
  const output = pstfx(SOURCE, [scanlines({ animate: true })], { animate: false })
  assert.equal(count(output, '@keyframes'), 0)
})

test('rejects values that are not effects', () => {
  assert.throws(
    () => pstfx(SOURCE, [{ nope: true } as unknown as ReturnType<typeof blur>]),
    (error: PstfxError) => error.code === 'INVALID_EFFECT',
  )
})
...
test('supports custom filter effects', () => {
  ...
  assert.match(pstfx(SOURCE, [sepiaish]), /type="saturate" values="0.2"/)
})
...
test('supports custom layer effects that contribute defs and styles', () => {
  ...
  const output = pstfx(SOURCE, [badge])
  assert.match(output, /<defs><circle id="pstfx-[^"]+-dot-0"/)
