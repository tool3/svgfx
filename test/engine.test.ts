...
import type { SvgfxError } from '../src/core/errors.ts'
...
test('returns the document untouched when no effects are given', () => {
  assert.equal(svgfx(SOURCE, []), SOURCE)
})

test('adds the svg namespace when it is missing', () => {
  assert.match(svgfx('<svg viewBox="0 0 4 4"><rect/></svg>', []), /xmlns="http:\/\/www\.w3\.org\/2000\/svg"/)
})

test('is deterministic across identical calls', () => {
  const first = svgfx(SOURCE, [scanlines({ animate: true })])
  const second = svgfx(SOURCE, [scanlines({ animate: true })])
  assert.equal(first, second)
})

test('gives different documents different id namespaces', () => {
  const first = svgfx(SOURCE, [blur()])
  const second = svgfx(SOURCE.replace('100', '120'), [blur()])
  const idOf = (markup: string): string => /id="([^"]+)"/.exec(markup)?.[1] ?? ''
  assert.notEqual(idOf(first), idOf(second))
})

test('honours an explicit scope', () => {
  assert.match(svgfx(SOURCE, [blur()], { scope: 'hero' }), /id="svgfx-hero-filter-0"/)
})

test('honours a custom prefix', () => {
  assert.match(svgfx(SOURCE, [blur()], { prefix: 'acme' }), /id="acme-/)
})

test('merges consecutive filter effects into a single filter element', () => {
  const output = svgfx(SOURCE, [grayscale(), invert(), blur()])
...
test('chains filter primitives through the merged filter', () => {
  const output = svgfx(SOURCE, [grayscale(), invert()])
...
test('disables motion globally through settings', () => {
  const output = svgfx(SOURCE, [scanlines({ animate: true })], { animate: false })
  assert.equal(count(output, '@keyframes'), 0)
})

test('rejects values that are not effects', () => {
  assert.throws(
    () => svgfx(SOURCE, [{ nope: true } as unknown as ReturnType<typeof blur>]),
    (error: SvgfxError) => error.code === 'INVALID_EFFECT',
  )
})
...
test('supports custom filter effects', () => {
  ...
  assert.match(svgfx(SOURCE, [sepiaish]), /type="saturate" values="0.2"/)
})
...
test('supports custom layer effects that contribute defs and styles', () => {
  ...
  const output = svgfx(SOURCE, [badge])
  assert.match(output, /<defs><circle id="svgfx-[^"]+-dot-0"/)
